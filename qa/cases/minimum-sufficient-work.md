...
| MW-08: beneficial parallelism | Independent packets have distinct outputs and parallel execution could save time or context. | Without an explicit user request, stay in the current chat. Within authorized delegation, use disjoint ownership and compact receipts. |
...
| MW-14: selected flagship | User authorized a worker and selected a high-capability model and effort; dispatch without a justified override. | Preserve both settings through inheritance; do not downgrade or demand a separate selection justification. |
