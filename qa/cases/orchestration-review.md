...
| OR-03: unavailable proof | Mandatory runtime check cannot run; no failure is otherwise established. | `not_verified`, exact missing evidence and owner, then `waiting_evidence`. No acceptance, goal-level blocking, or implementation-defect claim from absence alone. |
