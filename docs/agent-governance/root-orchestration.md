...
Current preferred setting for these roles is Sol with high reasoning. Use Max
only for an exceptionally difficult single-agent reasoning task.
...
Current preferred setting for these roles is Terra with medium reasoning,
raising it to high when the packet contains meaningful judgment.

Never downgrade a worker solely to save tokens when doing so creates a
material quality risk.

Do not use automatic fan-out or Ultra as the default execution model for an
explicitly orchestrated goal. `/root` owns every spawn, packet, and acceptance
decision.
