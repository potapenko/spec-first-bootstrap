...
## PT.PACKET.CONTEXT — Finite routed worker context
...
- Route Receipt and pinned contract revisions;
...
An implementation worker does not reinterpret product intent. Missing routed
...
envelope, route receipts, revisions, protected domains, accepted evidence, and
...
Every product packet pins route revisions, contract revisions, and clause IDs.
...
layers, the current objective and envelope, the latest Route Receipt, selected
route manifests, pinned contract closure, revisions, accepted deltas,
unresolved discrepancies, and only the evidence/QA instructions needed next.

Rerun mechanical route resolution. If manifest or contract revisions differ,
record revision drift and re-establish the Spec Basis before product action. If
the task or domain changed, traverse from the root router again. Do not reread
...
A bounded discrepancy receipt states packet, routed revisions and clauses,
...
revision, routed contract revisions, included domains/clauses, QA/runtime
