...
1. Detect the active agent environment and its supported user-level
   instruction entry point. Do not assume another user's path or silently
...
Do not expose credentials, tokens, provider configuration, or unrelated global
settings.
...
the checkpoint directive: before committing, the agent verifies a safe, writable
...
the Markdown traversal receipt, pinned contract closure, specified expectation, observed
